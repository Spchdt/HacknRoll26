...
export default function GitGraphComponent({ graph, files, className }: GitGraphProps) {
  ...
  const { nodes, connections, lanes, dimensions } = useMemo(() => {
    const nodePositions = new Map<string, NodePosition>();
    const connectionsList: { from: NodePosition; to: NodePosition; color: string }[] = [];
    
    // Only show branches that exist in the graph (don't show file target branches until created)
  ...
  // File positions - only show files on branches that exist
  const filePositions = useMemo(() => {
    return files
      .filter((file) => {
        // Only show file if its branch exists in the graph
        return graph.branches.has(file.branch);
      })
      .map((file) => {
  ...
  }, [files, nodes, lanes, graph.branches]);

  return (
    <div className={cn('relative bg-gray-50 rounded-lg overflow-auto border border-gray-200 p-2', className)}>
